...
(function(process) {
        ...
        if (!this._selected && this.parentNode) {
          ...
          if (select) {
            ...
            if (options.item(0) === this && !select.hasAttribute('multiple')) {
              var found = false,
                  optArray = options._toArray();
              for (var i = 1,
                  l = optArray.length; i < l; i++) {
  ...
  define('HTMLCanvasElement', {
    ...
    elementBuilder: function(element) {
      try {
        var canvas = new (require('@empty'))(0, 0);
  ...
  define('HTMLScriptElement', {
    ...
    proto: {
      _eval: function(text, filename) {
        if (this._ownerDocument.implementation.hasFeature("ProcessExternalResources", "script") && this.language && core.languageProcessors[this.language]) {
        ...
        if (name === 'name') {
          ...
          if (!this._attachedToDocument) {
            if (!this._waitingOnInsert) {
              this.removeEventListener('DOMNodeInsertedIntoDocument', this._initInsertListener, false);
