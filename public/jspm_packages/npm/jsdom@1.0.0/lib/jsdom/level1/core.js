  ...
  if (src.attributes) {
    ...
    for (i = 0; i < l; i++) {
      ...
      if (attr.namespaceURI) {
        dest.setAttributeNS(attr.namespaceURI, attr.nodeName, attr.nodeValue);
...
core.Node.prototype = {
  ...
  get nodeName() {
    ...
    if (this.nodeType === ELEMENT_NODE && this._ownerDocument && this._ownerDocument._doctype && this._ownerDocument._doctype.name.toLowerCase().indexOf("html") !== -1) {
  ...
  _attach: function() {
    ...
    for (var i = 0,
        len = this._childNodes.length; i < len; i++) {
  ...
  cloneNode: function(deep, fn) {
    ...
    switch (this.nodeType) {
      ...
      case this.NOTATION_NODE:
        object = this._ownerDocument.createNotationNode(this._name, this._publicId, this._systemId);
...
AttributeList.prototype = {
  ...
  $removeNode: function(attr) {
    ...
    if (doc && doc._doctype && doc._doctype.name.toLowerCase() !== "html") {
      ...
      if (elem) {
        var defaultValue = elem.attributes.getNamedItemNS(attr._namespaceURI, attr._localName);
        if (defaultValue) {
          this.$set(attr._localName, defaultValue.value, attr.name, attr._prefix, attr._namespaceURI);
...
inheritFrom(core.Node, core.Element, {
  ...
  getElementsByTagName: core.memoizeQuery(function(name) {
    name = name.toLowerCase();
    function filterByTagName(child) {
      child = (child.nodeType === ENTITY_REFERENCE_NODE) ? child._entity : child;
...
inheritFrom(core.Node, core.Document, {
  ...
  createEntityNode: function(name) {
  ...
  createNotationNode: function(name, publicId, systemId) {
    if (name.match(entRegEx) || !name || !name.length) {
      throw new core.DOMException(INVALID_CHARACTER_ERR);
    }
...
inheritFrom(core.Node, core.CharacterData, {
  ...
  get length() {
    return this._nodeValue.length || 0;
  },
  substringData: function(offset, count) {
    if (count < 0 || offset < 0 || offset > this._nodeValue.length) {
      throw new core.DOMException(INDEX_SIZE_ERR);
    }
    return (this._nodeValue.length < offset + count) ? this._nodeValue.substring(offset) : this._nodeValue.substring(offset, offset + count);
  },
  ...
  deleteData: function(offset, count) {
    ...
    if (offset < 0 || offset > this._nodeValue.length || count < 0) {
      throw new core.DOMException(INDEX_SIZE_ERR);
    }
    var start = this._nodeValue.substring(0, offset);
    this._nodeValue = (offset + count < this._nodeValue.length) ? start + this._nodeValue.substring(offset + count) : start;
  },
  replaceData: function(offset, count, arg) {
    if (this._readonly === true) {
      throw new core.DOMException(NO_MODIFICATION_ALLOWED_ERR);
    }
    count = (offset + count > this._nodeValue.length) ? this.nodeValue.length - offset : count;
    if (offset < 0 || offset > this._nodeValue.length || count < 0) {
      throw new core.DOMException(INDEX_SIZE_ERR);
    }
    var start = this._nodeValue.substring(0, offset);
...
inheritFrom(core.Text, core.Comment, {nodeType: COMMENT_NODE});
core.CDATASection = function CDATASection(document, value) {
  core.Text.call(this, document, value);
  this._nodeName = "#cdata-section";
};
inheritFrom(core.Text, core.CDATASection, {nodeType: CDATA_SECTION_NODE});
...
inheritFrom(core.Node, core.Notation, {
  ...
  get name() {
    return this._name || this._nodeName;
  },
  get attributes() {
    return null;
  },
...
inheritFrom(core.Node, core.EntityReference, {
  ...
  get nodeValue() {
    return (this._entity) ? this._entity.nodeValue : null;
  },
  set nodeValue(unused) {
    if (this._readonly === true) {}
  },
  ...
  get childNodes() {
    return this._entity.childNodes;
  },
  get lastChild() {
